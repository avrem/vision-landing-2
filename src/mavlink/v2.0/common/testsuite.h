...
static void mavlink_test_gen_status(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_GEN_STATUS >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_gen_status_t packet_in = {
        17.0,963497672,73.0,101.0,53,120,187,254,157.0,963498712,213.0
    };
    mavlink_gen_status_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.charge_current = packet_in.charge_current;
        packet1.rpm = packet_in.rpm;
        packet1.ice_temp = packet_in.ice_temp;
        packet1.gen_temp = packet_in.gen_temp;
        packet1.fuel_remaining = packet_in.fuel_remaining;
        packet1.cooler = packet_in.cooler;
        packet1.starter = packet_in.starter;
        packet1.throttle = packet_in.throttle;
        packet1.vsi_temp = packet_in.vsi_temp;
        packet1.rpm_alt = packet_in.rpm_alt;
        packet1.ice_temp_alt = packet_in.ice_temp_alt;
        
        
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_GEN_STATUS_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_GEN_STATUS_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_gen_status_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_gen_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_gen_status_pack(system_id, component_id, &msg , packet1.charge_current , packet1.rpm , packet1.ice_temp , packet1.gen_temp , packet1.fuel_remaining , packet1.cooler , packet1.starter , packet1.throttle , packet1.vsi_temp , packet1.rpm_alt , packet1.ice_temp_alt );
    mavlink_msg_gen_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_gen_status_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.charge_current , packet1.rpm , packet1.ice_temp , packet1.gen_temp , packet1.fuel_remaining , packet1.cooler , packet1.starter , packet1.throttle , packet1.vsi_temp , packet1.rpm_alt , packet1.ice_temp_alt );
    mavlink_msg_gen_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_gen_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_gen_status_send(MAVLINK_COMM_1 , packet1.charge_current , packet1.rpm , packet1.ice_temp , packet1.gen_temp , packet1.fuel_remaining , packet1.cooler , packet1.starter , packet1.throttle , packet1.vsi_temp , packet1.rpm_alt , packet1.ice_temp_alt );
    mavlink_msg_gen_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("GEN_STATUS") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_GEN_STATUS) != NULL);
#endif
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_gen_status(system_id, component_id, last_msg);
